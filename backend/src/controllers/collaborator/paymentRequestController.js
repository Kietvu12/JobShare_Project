    ...
    try {
      upload(req, res, async (err) => {
        ...
        try {
          ...
          if (Number(jobApplication.status) < 14) {
            if (req.file) {
              try {
                await fs.unlink(req.file.path);
              } catch (unlinkError) {
            ...
            return res.status(403).json({
              success: false,
              message: 'Chỉ có thể gửi yêu cầu thanh toán sau khi ứng viên đã vào công ty'
            });
          }
          ...
          try {
            await collaboratorNotificationService.notifyAdminsPaymentRequestCreated({
              candidateName: paymentRequest.jobApplication?.cv?.name || null,
              jobCode: paymentRequest.jobApplication?.job?.jobCode || String(jobApplicationId),
              paymentRequestId: paymentRequest.id,
              jobApplicationId: parseInt(jobApplicationId, 10)
            });
          } catch (notifyErr) {
            console.error('[Payment Request] Error notifying admins:', notifyErr);
          }
