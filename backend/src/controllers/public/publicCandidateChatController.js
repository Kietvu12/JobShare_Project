...
import { enrichPublicChatMessage, markVisitorPublicChatSessionRead } from '../../services/publicChatReadService.js';
...
export const publicCandidateChatController = {
  ...
  getMessages: async (req, res, next) => {
    try {
      ...
      res.json({
        success: true,
        data: { messages: messages.map((m) => serializeMessage(m, session)) }
      });
    } catch (error) {
      next(error);
    }
