  ...
  getCollaborators: async (req, res, next) => {
    try {
      ...
      if (collaboratorIds.length > 0) {
        const validCandidatesCounts = await sequelize.query(
          ...
          {
            replacements: { collaboratorIds },
            type: sequelize.QueryTypes.SELECT,
          }
        );

        const validCountMap = {};
        validCandidatesCounts.forEach((row) => {
          validCountMap[row.collaborator_id] = parseInt(row.count, 10) || 0;
        });

        const interviewingCounts = await sequelize.query(
          `SELECT collaborator_id, COUNT(*) as count
        ...
        const interviewingCountMap = {};
        interviewingCounts.forEach((item) => {
          interviewingCountMap[item.collaborator_id] = parseInt(item.count, 10) || 0;
        });

        rows.forEach((collaborator) => {
          collaborator.dataValues.validApplicationsCount = validCountMap[collaborator.id] || 0;
          collaborator.dataValues.interviewingApplicationsCount = interviewingCountMap[collaborator.id] || 0;
          collaborator.dataValues.applicationsCount = collaborator.dataValues.interviewingApplicationsCount;
        });
      } else {
        rows.forEach((collaborator) => {
          collaborator.dataValues.validApplicationsCount = 0;
          collaborator.dataValues.interviewingApplicationsCount = 0;
