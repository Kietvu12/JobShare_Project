...
const AI_PARSE_URL = 'https://test.ws-jobshare.com/api_ai/v2/parser/cv';
      ...
      try {
        ...
        if (keys.length > 0) {
          ...
          for (let i = 0; i < keys.length; i++) {
            ...
            const srcDisplayName = await getObjectOriginalDisplayName(srcKey);
            const { storageName, displayName } = toSafeStorageFilename(srcDisplayName, i, usedNames);
            const destKey = `${destOriginalFolderKey}/${storageName}`;
            const mime = obj.ContentType || getMimeTypeByFilename(storageName);
            const metaValue = encodeOriginalFilenameMetadata(displayName);
            await uploadBufferToS3(buf, destKey, mime, metaValue
    ...
    if (!copiedAny) {
      try {
        ...
        for (const p of candidates) {
          ...
          if (!stat) continue;
          if (stat.isFile()) {
            const fileName = toSafeStorageFilename(path.basename(p), usedNames.size, usedNames).storageName;
            await fs.copyFile(abs, path.join(origDir, fileName));
          } else if (stat.isDirectory()) {
            const entries = await fs.readdir(abs);
            for (let i = 0; i < entries.length; i++) {
              const fileName = toSafeStorageFilename(entries[i], usedNames.size, usedNames).storageName;
              await fs.copyFile(path.join(abs, entries[i]), path.join(origDir, fileName));
            }
          }
        }
      } catch (e) {
