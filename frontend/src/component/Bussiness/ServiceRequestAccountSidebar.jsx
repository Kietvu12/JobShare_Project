  ...
  return (
    <aside className="flex min-h-0 flex-col gap-2 lg:h-full lg:overflow-hidden">
      <div className={`${CARD} shrink-0 p-2.5 sm:p-3`}>
        <h3 className="mb-2 text-xs font-bold text-slate-900">Tổng quan tài khoản</h3>
          ...
          {summaryItems.map((item, i) => {
            ...
            return (
              <li key={item.label} className="flex items-center gap-2">
                ...
                <span className="min-w-0 flex-1 text-[10px] leading-snug text-slate-500">{item.label}</span>
                <span className="shrink-0 text-[11px] font-semibold text-slate-900">{item.value}</span>
              </li>
            );
          })}
        </ul>
      ...
      <div className={`${CARD} flex min-h-0 flex-1 flex-col p-2.5 sm:p-3 lg:overflow-hidden`}>
        ...
        <div className="business-homepage-scroll min-h-0 flex-1 space-y-2 overflow-y-auto">
        ...
        <button
          type="button"
          onClick={() => navigate('/business/billing')}
          className="mt-2 w-full shrink-0 rounded-lg border border-slate-200 bg-slate-50 py-1.5 text-[10px] font-semibold text-slate-600 transition-colors hover:bg-slate-100"
        >
          Xem tất cả yêu cầu
        </button>
      </div>
