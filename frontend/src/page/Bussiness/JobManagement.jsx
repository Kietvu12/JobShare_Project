...
import {
  ...
  MapPin, Briefcase, ChevronLeft, ChevronRight, Sparkles, Loader2,
} from 'lucide-react'
...
function mapApiJobToRow(job, index) {
...
const JobManagement = () => {
  ...
  useEffect(() => {
    loadJobs(1)
    loadStatusCounts()
  }, [loadJobs, loadStatusCounts])

  const tableRows = useMemo(() => jobs.map((job, i) => mapApiJobToRow(job, i)), [jobs])
            ...
            <div className="bg-white rounded-xl border border-slate-100 flex flex-col justify-between" style={{ padding: '10px 12px' }}>
              <div>
                ...
                <p style={{ fontSize: 10, color: '#475569', marginBottom: 3 }}>
                  Ước tính có <span style={{ fontSize: 16, fontWeight: 700, color: '#1e293b' }}>128</span>{' '}
                  <span style={{ fontWeight: 600 }}>ứng viên</span> phù hợp
                </p>
                <p style={{ fontSize: 10, color: '#475569', marginTop: 6 }}>
                  Chất lượng ứng viên:{' '}
                  <span style={{ fontWeight: 600, color: '#f59e0b' }}>Trung bình khá</span>
                  <span style={{ color: '#cbd5e1', marginLeft: 3 }}>ⓘ</span>
                </p>
              </div>
              <button className="flex items-center justify-center gap-1 border border-blue-200 text-blue-600 hover:bg-blue-50 rounded-lg font-semibold transition-colors" style={{ marginTop: 10, width: '100%', padding: '6px 8px', fontSize: 10 }}>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={{ padding: '7px 10px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                            ...
                            <button type="button" className="text-slate-400 hover:text-slate-600" style={{ marginLeft: 4, fontSize: 13 }} onClick={(e) => e.stopPropagation()}>⋮</button>
                          </td>
                        </tr>
                      )
                    })
