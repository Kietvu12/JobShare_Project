...
import { formatJobCommissionSummary } from '../../utils/jobCommissionUi'
...
function CreateListingModal({ open, onClose, onCreated }) {
  ...
  useEffect(() => {
    ...
    apiService.getBusinessJobById(jobId).then((res) => {
      ...
      setCommissionPreview(job ? formatJobCommissionSummary(job) : 'Liên hệ')
    }).catch(() => {
      if (mounted) setCommissionPreview('Liên hệ')
    })
    return () => { mounted = false }
  }, [jobId])
        ...
        <div className="p-4 space-y-3 text-sm">
          ...
          {jobId && (
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <div className="text-xs font-semibold text-slate-600 mb-1">Phí thưởng CTV (theo JD đã cài)</div>
              <pre className="text-xs text-slate-700 whitespace-pre-wrap font-sans leading-relaxed">{commissionPreview || 'Đang tải...'}</pre>
              <p className="text-[10px] text-slate-500 mt-2">Mức phí lấy từ phần &quot;Cài đặt phí&quot; khi tạo job — không nhập lại ở đây.</p>
            </div>
          )}
...
const CandidateSharing = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [dashRes, listRes, nomRes, setRes] = await Promise.all([
        ...
        apiService.getBusinessCandidateSharingNominations({ page: 1, limit: 10 }),
  ...
  useEffect(() => {
    if (nominations.length && !selectedNomination) setSelectedNomination(nominations[0])
  }, [nominations, selectedNomination])
  ...
  return (
    <>
      ...
      <CreateListingModal open={showCreate} onClose={() => setShowCreate(false)} onCreated={loadData} />
              ...
              <table style={{ width: '100%', fontSize: 8, borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {nominationsData.length === 0 ? (
                    ...
                    const sel = selectedNomination?.nominationId === n.nominationId
                    return (
                      <tr
                        key={n.nominationId}
