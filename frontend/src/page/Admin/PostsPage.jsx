...
export default function PostsPage() {
  ...
  const htmlImageInputRef = useRef(null);
  const htmlContentTextareaRef = useRef(null);
  const [eventsList, setEventsList] = useState([]);
  const [eventsLoading, setEventsLoading] = useState(false);
  const [contentEditorMode, setContentEditorMode] = useState('rich'); // 'rich' | 'html'
  const [uploadingHtmlImage, setUploadingHtmlImage] = useState(false);
  const [htmlUploadedAssets, setHtmlUploadedAssets] = useState([]);
  ...
  const stripHtmlForExcerpt = (html) => {
    if (!html || typeof html !== 'string') return '';
    return html.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
  };

  const buildPostSavePayload = (data, extra = {}) => {
    const merged = { ...data, ...extra };
    const next = { ...merged };

    if (merged.title?.trim()) next.metaTitle = merged.title.trim();
    if (merged.titleEn?.trim()) next.metaTitleEn = merged.titleEn.trim();
    if (merged.titleJa?.trim()) next.metaTitleJa = merged.titleJa.trim();

    if (merged.thumbnail) next.metaImage = merged.thumbnail;
  ...
  const getActiveContentField = () => {
    if (activeLang === 'en') return 'contentEn';
    if (activeLang === 'ja') return 'contentJa';
    return 'content';
  };

  const insertHtmlSnippet = (snippet) => {
    const field = getActiveContentField();
    const textarea = htmlContentTextareaRef.current;
    ...
    if (textarea) {
      ...
      setFormData((prev) => ({ ...prev, [field]: current + snippet }));
    }
  };
  ...
  const handleHtmlImageUpload = async (e) => {
    ...
    try {
      ...
      const res = isEdit
        ? await apiService.uploadPostImage(id, compressed)
        : await apiService.uploadPostTempImage(compressed);
      if (res?.success && res?.data) {
        const url = res.data.url || normalizePostImageUrl(res.data.key);
        const snippet = `\n<img src="${url}" alt="" />\n`;
        insertHtmlSnippet(snippet);
        setHtmlUploadedAssets((prev) =>
          [{ url, key: res.data.key, name: file.name }, ...prev].slice(0, 12)
        );
      }
    } catch (err) {
      alert('Lỗi upload ảnh HTML: ' + (err.message || ''));
    } finally {
      setUploadingHtmlImage(false);
      if (htmlImageInputRef.current) htmlImageInputRef.current.value = '';
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    savePost(buildPostSavePayload(formData, { slug, tag: tagList.join(', ') }));
  };
  ...
  const handleSaveDraft = () => {
    ...
    savePost(buildPostSavePayload(formData, {
      slug,
      status: STATUS_DRAFT,
      tag: tagList.join(', '),
    }));
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="lg:col-span-2 flex flex-col gap-5">
                ...
                <div className="rounded-xl border bg-white overflow-hidden" style={{ borderColor: '#e5e7eb' }}>
                  <div className="px-4 py-2.5 border-b flex items-center justify-between gap-3" style={{ borderColor: '#e5e7eb' }}>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        disabled={contentEditorMode === 'rich'}
                        className="px-3 py-1 rounded-full text-[11px] font-semibold border transition-colors disabled:opacity-60"
                        style={{
                          borderColor: '#e5e7eb',
                          color: contentEditorMode === 'rich' ? '#fff' : '#374151',
                          backgroundColor: contentEditorMode === 'rich' ? '#dc2626' : '#fff',
                        }}
                      >
                        RichText
                      </button>
                      <button
                        ...
                        style={{
                          borderColor: '#e5e7eb',
                          color: contentEditorMode === 'html' ? '#fff' : '#374151',
                          backgroundColor: contentEditorMode === 'html' ? '#dc2626' : '#fff',
                        }}
                      >
                        HTML
                      </button>
                      ...
                      <div className="p-4">
                        ...
                        <div className="mb-3 flex flex-wrap items-center gap-2">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold border transition-colors disabled:opacity-50"
                            style={{ borderColor: '#e5e7eb', color: '#374151', backgroundColor: '#fff' }}
                          >
                            <ImagePlus className="w-3.5 h-3.5" />
                            {uploadingHtmlImage ? 'Đang upload...' : 'Upload ảnh'}
                          </button>
                          <span className="text-[11px] text-gray-500">
                            Ảnh upload sẽ được chèn vào vị trí con trỏ dưới dạng thẻ &lt;img&gt;
                          </span>
                        </div>
                        <textarea
                          ...
                          onChange={(e) => {
                            ...
                            setFormData((prev) => ({
                              ...prev,
                              ...(activeLang === 'vi' && { content: nextVal }),
                              ...(activeLang === 'en' && { contentEn: nextVal }),
                              ...(activeLang === 'ja' && { contentJa: nextVal }),
                            }));
                          }}
                          placeholder={t.postsContentPlaceholder || 'Dán HTML vào đây...'}
                          className="w-full rounded-lg border min-h-[260px] px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-red-500/30"
                          style={{ borderColor: '#e5e7eb', fontFamily: 'monospace' }}
                        />
                        {htmlUploadedAssets.length > 0 && (
                          <div className="mt-3 rounded-lg border p-3" style={{ borderColor: '#e5e7eb', backgroundColor: '#fafafa' }}>
                            <p className="text-[11px] font-semibold text-gray-600 mb-2">Ảnh đã upload (click để chèn lại)</p>
                            <div className="flex flex-wrap gap-2">
                              {htmlUploadedAssets.map((asset) => (
                                <button
                                  key={asset.key || asset.url}
                                  type="button"
                                  onClick={() => insertHtmlSnippet(`\n<img src="${asset.url}" alt="" />\n`)}
                                  className="group relative h-14 w-14 overflow-hidden rounded-md border bg-white"
                        ...
                        <p className="mt-2 text-[11px] text-gray-500">
                          Dán HTML trực tiếp hoặc upload ảnh. Nếu đổi qua RichText rồi quay lại, nội dung có thể bị làm sạch theo Quill.
                        </p>
                      </div>
                    ) : (
                      <RichTextEditor
