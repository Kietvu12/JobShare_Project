    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      ...
      {!isCreate && business && (
        <div className={`rounded-xl border px-4 py-3 text-sm ${
          ...
          {business.emailVerifiedAt
            ? `Email đã xác thực (${new Date(business.emailVerifiedAt).toLocaleString('vi-VN')})`
            : 'Email chưa xác thực — DN không thể đăng nhập cho đến khi xác thực email hoặc admin chọn trạng thái «Hoạt động» và lưu.'}
          {business.approvedAt && (
            <span className="block mt-1 text-xs opacity-80">
              Duyệt lúc: {new Date(business.approvedAt).toLocaleString('vi-VN')}
            </span>
          )}
        </div>
      )}
