...
import {
  ...
  extractCollaboratorsFromAssignments,
  rankCollaboratorsBySearch,
  mergeCollaboratorLists,
} from '../../utils/collaboratorSearch';
...
const AdminAddNominationPage = () => {
  ...
  const [collaboratorsLoadingMore, setCollaboratorsLoadingMore] = useState(false);
  ...
  const fetchCollaboratorPage = useCallback(async ({ page = 1, append = false, query = '' } = {}) => {
    ...
    if (isAdminBackOffice) {
      ...
      if (response.success && response.data) {
        list = extractCollaboratorsFromAssignments(response);
      }
    } else {
      ...
      if (response.success && response.data) {
        list = normalizeCollaboratorList(response).filter((c) => c.status === 1);
      }
    }

    const ranked = rankCollaboratorsBySearch(q, list);
    setCollaborators((prev) => (append ? mergeCollaboratorLists(prev, ranked) : ranked));
    setCollaboratorHasMore(list.length >= limit);
    setCollaboratorLastQuery(q);
  }, [adminProfile]);

  useEffect(() => {
    if (adminProfile === null || !showCollaboratorDropdown) return undefined;
    const timer = setTimeout(async () => {
      setCollaboratorsLoading(true);
      setCollaboratorPage(1);
      setCollaboratorHasMore(true);
      try {
        await fetchCollaboratorPage({ page: 1, append: false, query: collaboratorSearch });
      } catch (error) {
  ...
  }, [adminProfile, showCollaboratorDropdown, collaboratorSearch, fetchCollaboratorPage]);

  useEffect(() => {
    const el = collaboratorDropdownLoadMoreRef.current;
    if (!el || !showCollaboratorDropdown || !collaboratorHasMore || collaboratorsLoading || collaboratorsLoadingMore) {
      return undefined;
    }
    const observer = new IntersectionObserver((entries) => {
      if (!entries[0]?.isIntersecting) return;
      if (collaboratorsLoadingMore || !collaboratorHasMore) return;
      setCollaboratorsLoadingMore(true);
      ...
      fetchCollaboratorPage({ page: nextPage, append: true, query: collaboratorLastQuery })
        ...
        .finally(() => setCollaboratorsLoadingMore(false));
    ...
    collaboratorsLoadingMore,
    collaboratorPage,
    collaboratorLastQuery,
    fetchCollaboratorPage,
  ]);

  // Load candidates (toàn hệ thống hoặc lọc theo CTV) khi tab "có sẵn" + debounce tìm kiếm
  useEffect(() => {
    if (isEditMode || candidateTab !== 'existing') return;
    const timer = setTimeout(() => {
      loadCandidates(formData.collaboratorId || null, candidateSearch);
    }, candidateSearch.trim() ? 300 : 0);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData.collaboratorId, candidateSearch, candidateTab, isEditMode]);
  ...
  useEffect(() => {
    if (!showCollaboratorDropdown) return;
    const handleClickOutside = (e) => {
                  ...
                  <div>
                    <label className="block text-[10px] sm:text-xs font-semibold mb-1" style={{ color: '#111827' }}>
                      ...
                      <span className="text-[10px] font-normal ml-1" style={{ color: '#6b7280' }}>
                        (tùy chọn — lọc ứng viên theo CTV)
                      </span>
                    </label>
                    <div className="relative" ref={collaboratorDropdownRef}>
                      <div className="flex items-center px-3 py-1.5 rounded-full bg-white text-[12px] sm:text-xs min-w-0 border" style={{ borderColor: '#e5e7eb' }}>
                        <Users className="w-3.5 h-3.5 mr-2 flex-shrink-0" style={{ color: '#9ca3af' }} />
                        <input
                          type="text"
                        ...
                        {formData.collaboratorId && (
                          <button
                            type="button"
                            onClick={handleCollaboratorClear}
                      ...
                      {showCollaboratorDropdown && (
                        <div className="absolute z-30 w-full mt-2 rounded-xl border p-3 max-h-48 overflow-y-auto text-[11px] sm:text-xs" style={{ backgroundColor: 'white', borderColor: '#e5e7eb' }}>
                          {collaboratorsLoading ? (
                            <div className="px-2 py-2" style={{ color: '#6b7280' }}>Đang tải...</div>
                          ) : collaborators.length === 0 ? (
                            <div className="px-2 py-2" style={{ color: '#6b7280' }}>
                              {collaboratorSearch.trim() ? 'Không tìm thấy CTV' : 'Chưa có CTV'}
                            </div>
                          ) : (
                            <>
                              {collaborators.map((collaborator, index) => (
                                ...
                                >
                                  ...
                                  <div style={{ color: '#6b7280' }}>{collaborator.code || collaborator.id} • {collaborator.email}</div>
                                </button>
                              ))}
                              <div ref={collaboratorDropdownLoadMoreRef} className="h-6 flex items-center justify-center" style={{ color: '#9ca3af' }}>
                                {collaboratorsLoadingMore ? 'Đang tải thêm...' : collaboratorHasMore ? '' : null}
                              </div>
                            </>
                          )}
                        </div>
                    ...
                    {formData.collaboratorId && (
                      <div className="mt-2 p-2 border rounded-lg" style={{ backgroundColor: '#f9fafb', borderColor: '#e5e7eb' }}>
                        <div className="text-xs font-medium" style={{ color: '#374151' }}>Đã chọn CTV: {formData.collaboratorName}</div>
                      </div>
                    )}
                  </div>
                )}
                {formData.collaboratorId && candidates.length === 0 && !candidatesLoading && (
                ...
                {!isEditMode && (
                  <div>
                    <label className="block text-[10px] sm:text-xs font-semibold mb-1" style={{ color: '#111827' }}>
                      ...
                      <span className="text-[10px] font-normal ml-1" style={{ color: '#6b7280' }}>
                        {formData.collaboratorId
                          ? `(CTV: ${formData.collaboratorName})`
                          : '(toàn hệ thống)'}
                        {candidatesLoading ? ' — đang tải...' : ` — ${candidates.length} kết quả`}
                      </span>
                    </label>
                    <div className="relative" ref={candidateDropdownRef}>
                      ...
                      {showCandidateDropdown && candidatesLoading && (
                        <div className="absolute z-20 w-full mt-2 rounded-xl border p-3" style={{ backgroundColor: 'white', borderColor: '#e5e7eb' }}>
                          <p className="text-[11px] sm:text-xs" style={{ color: '#6b7280' }}>Đang tìm ứng viên...</p>
                        </div>
                      )}
                      {showCandidateDropdown && !candidatesLoading && filteredCandidates.length > 0 && (
                      ...
                      {showCandidateDropdown && !candidatesLoading && filteredCandidates.length === 0 && candidateSearch && (
                        <div className="absolute z-20 w-full mt-2 rounded-xl border p-3" style={{ backgroundColor: 'white', borderColor: '#e5e7eb' }}>
                          <p className="text-[11px] sm:text-xs" style={{ color: '#6b7280' }}>Không tìm thấy ứng viên nào phù hợp với từ khóa "{candidateSearch}"</p>
                        </div>
                      )}
                      {showCandidateDropdown && !candidatesLoading && candidates.length === 0 && !candidateSearch && (
                        <div className="absolute z-20 w-full mt-2 rounded-xl border p-3" style={{ backgroundColor: 'white', borderColor: '#e5e7eb' }}>
                          <p className="text-[11px] sm:text-xs" style={{ color: '#6b7280' }}>
                            {formData.collaboratorId
                              ? 'CTV này chưa có ứng viên nào. Vui lòng tạo ứng viên mới hoặc bỏ lọc CTV.'
                              : 'Chưa có ứng viên. Nhập tên hoặc mã để tìm, hoặc tạo ứng viên mới.'}
                          </p>
                        </div>
                      )}
                    </div>
